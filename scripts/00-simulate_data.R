...
library(dplyr)
library(readr)  # Ensure readr is loaded for write_csv
...
teams_info <- data.frame(
  team_abbrev = c("BAL", "BOS", "CHW", "CLE", "DET", 
                  "HOU", "KC", "LAA", "MIN", 
                  "NYY", "OAK", "SEA", "TB", 
                  "TEX", "TOR",
                  "ARI", "ATL", "CHC", "CIN",
...
simulated_data <- team_data |>
  mutate(
    ...
    run_diff = ifelse(win_pct < 0.45,
                      ...
                      sample(0:200, nrow(team_data), replace = TRUE)),   # Positive run differential for higher win_pct
